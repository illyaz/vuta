...
public class SearchController : ControllerBase
{
        ...
        [FromQuery] string? channelId = null)
    {
        ...
        var result = await _elastic.SearchAsync<ESComment>(d => d
            ...
            .Sort(s =>
            {
                ...
                return s.Field(new Field("_score"), SortOrder.Descending);
            })
            .Highlight(h => h
                .Fields(fs => fs
                    .Field(f => f.Text)
        ...
        var videos = hitVideoIds.Any()
            ? await _db.Videos
                .AsNoTracking()
                .Where(x => hitVideoIds.Contains(x.Id))
                .Select(x => new
                    { x.Id, x.Title, x.PublishDate, x.ViewCount, x.LastUpdate, ChannelTitle = x.Channel.Title })
        ...
        return Ok(new
        {
            ...
            Hits = result.Hits
                .Select(hit => new
                {
                    hit.Id,
                    hit.Source.VideoId,
                    hit.Source.ChannelId,
                    videos[hit.Source.VideoId].ChannelTitle,
        ...
        [FromQuery] [Range(0, 399)] int page = 0)
    {
        ...
        var result = await _elastic.SearchAsync<ESChannel>(d => d
            ...
            .Sort(f => f.Field(new Field("_score"), SortOrder.Descending)));
        ...
        return Ok(new
        {
            ...
            Hits = result.Hits
                .Select(hit => new
                {
                    ...
                    SubscriberCount = hit.Source.SubscriberCount == null
                        ? null as long?
                        : long.Parse(hit.Source.SubscriberCount),
