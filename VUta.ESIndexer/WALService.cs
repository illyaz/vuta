...
{
    private static readonly Func<ReplicationValue, ValueTask<object?>> _dateTimeParse = async v =>
    {
        if (v.IsDBNull)
            return null;
    ...
    {
        _logger.LogInformation("Starting LogicalReplication({Replication}, {Publication})", _options.Replication,
            _options.Publication);
                           ...
                           slot, new PgOutputReplicationOptions(_options.Publication, 1), stoppingToken))
        {
            if (_logger.IsEnabled(LogLevel.Debug))
                _logger.LogDebug("WAL Received: [{ServerClock}] {Start} {Type}", message.ServerClock, message.WalStart,
            ...
            switch (message)
            {
                case InsertMessage insert when insert.Relation.Namespace == "public":
                    if (insert.Relation.RelationName == "videos")
                        await _indexer.AddDataAsync(
                            new ESIndexerData("videos", await ParseAsync(_videoFields, insert.Relation, insert.NewRow),
                                message.WalEnd), stoppingToken);
                    else if (insert.Relation.RelationName == "channels")
                        await _indexer.AddDataAsync(
                            new ESIndexerData("channels",
                                await ParseAsync(_channelFields, insert.Relation, insert.NewRow), message.WalEnd),
                            stoppingToken);
                    else if (insert.Relation.RelationName == "comments")
                        await _indexer.AddDataAsync(
                            new ESIndexerData("comments",
                                await ParseAsync(_commentFields, insert.Relation, insert.NewRow), message.WalEnd),
                            stoppingToken);
                    break;
                case UpdateMessage update when update.Relation.Namespace == "public":
                    if (update.Relation.RelationName == "videos")
                        await _indexer.AddDataAsync(
                            new ESIndexerData("videos", await ParseAsync(_videoFields, update.Relation, update.NewRow),
                                message.WalEnd), stoppingToken);
                    else if (update.Relation.RelationName == "channels")
                        await _indexer.AddDataAsync(
                            new ESIndexerData("channels",
                                await ParseAsync(_channelFields, update.Relation, update.NewRow), message.WalEnd),
                            stoppingToken);
                    else if (update.Relation.RelationName == "comments")
                        await _indexer.AddDataAsync(
                            new ESIndexerData("comments",
                                await ParseAsync(_commentFields, update.Relation, update.NewRow), message.WalEnd),
                            stoppingToken);
                    break;
                case KeyDeleteMessage delete when delete.Relation.Namespace == "public":
                    if (delete.Relation.RelationName == "videos")
                        await _indexer.AddDataAsync(
                            new ESIndexerData("videos", await ParseAsync(_idFields, delete.Relation, delete.Key),
                                message.WalEnd, true), stoppingToken);
                    else if (delete.Relation.RelationName == "channels")
                        await _indexer.AddDataAsync(
                            new ESIndexerData("channels", await ParseAsync(_idFields, delete.Relation, delete.Key),
                                message.WalEnd, true), stoppingToken);
                    else if (delete.Relation.RelationName == "comments")
                        await _indexer.AddDataAsync(
                            new ESIndexerData("comments", await ParseAsync(_idFields, delete.Relation, delete.Key),
                                message.WalEnd, true), stoppingToken);
                    break;
            }
        }
        ...
        ReplicationTuple rows)
    {
        ...
        if (data.Count != fields.Count)
            throw new InvalidOperationException(
                $"Expected field not exists ({string.Join(",", fields.Keys.Except(data.Keys))})");
