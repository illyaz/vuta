...
public class SyncService : BackgroundService
{
    ...
    private async Task IndexChannelAsync(VUtaDbContext db, CancellationToken stoppingToken)
    {
                           ...
                           .WithCancellation(stoppingToken))
        {
            ...
            await _indexer.AddDataAsync(new ESIndexerData("channels", new Dictionary<string, object?>
            {
                { "id", channel.Id },
                { "title", channel.Title },
    ...
    private async Task IndexVideoAsync(VUtaDbContext db, CancellationToken stoppingToken)
    {
                           ...
                           .WithCancellation(stoppingToken))
        {
            ...
            await _indexer.AddDataAsync(new ESIndexerData("videos", new Dictionary<string, object?>
            {
                { "id", video.Id },
    ...
    private async Task IndexCommentAsync(VUtaDbContext db, CancellationToken stoppingToken)
    {
                           ...
                           .WithCancellation(stoppingToken))
        {
            ...
            await _indexer.AddDataAsync(new ESIndexerData("comments", new Dictionary<string, object?>
            {
                { "id", comment.Id },
